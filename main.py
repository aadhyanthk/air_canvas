# -*- coding: utf-8 -*-
"""
Created on Tue Sep 20 15:11:18 2022
...
with mp_hands.Hands(min_detection_confidence=0.8, min_tracking_confidence=0.5) as hands: 
    while cap.isOpened():
        ...
        for i in range(len(points)):
            ...
            if (i > 0 and dist < 100 and tdist < 2):
        ...
        for i in range(1, len(pointsmo)):
            #print(points[i][0], points[i][1])
            
            #calculate distance between the points *IMP*
            
            dist = abs((pointsmo[i][0] - pointsmo[i-1][0]) + (pointsmo[i][1] - pointsmo[i-1][1]))
            tdist = pointsmo[i][2] - pointsmo[i-1][2]
            print(tdist)
            if (i > 0 and dist < 100 and tdist < 2):
                cv2.line(image, (int(pointsmo[i-1][0]), int(pointsmo[i-1][1])), (int(pointsmo[i][0]), int(pointsmo[i][1])),(121, 122, 76), 10)
            
            cv2.circle(image, (int(pointsmo[i][0]), int(pointsmo[i][1])), 1, (121, 122, 76), 1)
        ...
        if cv2.waitKey(10) & keyboard.is_pressed("y"):
            if(len(pointsmo) > 0):
                pointsmo .pop()
        ...
        #I will use it to make everything straight todo
        
        if cv2.waitKey(10) & keyboard.is_pressed("s"):
            if (straightLine):
                straightLine = False
            else:
                straightLine = True;
        ...
        if straightLine:
            ...
            for i in range(0,len(points)):
                points[i][0] = points[i][0] / oval
                points[i][0] = (int(points[i][0]) * oval) + oval/2
                points[i][1] = points[i][1] / oval
                points[i][1] = (int(points[i][1]) * oval) + oval/2
        
        if not straightLine:
            smoval = 2
            if len(pointsmo) % smoval == 0:
                #print("smooth")
                for i in range(0, len(pointsmo), smoval):
                    
                    if pointsmo[i+1][2] - pointsmo[i][2] > 2:
                        continue
                    
                    sumx = 0
                    sumy = 0
                    for j in range(0, smoval):
                                                
                        sumx += pointsmo[i+j][0]
                        sumy += pointsmo[i+j][1]
                    
                    for j in range(0, smoval):                        
                        pointsmo[i+j][0] = sumx/smoval
                        pointsmo[i+j][1] = sumy/smoval
